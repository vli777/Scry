...
class Config:
    def __init__(self, config_path="config.yaml"):
        ...
        self.raw_file = os.path.join(self.raw_dir, f"{self.symbol}_{self.frequency}min_data.parquet")
        self.scaler_file = os.path.join(self.scaler_dir, f"scaler_{self.symbol}.pkl")
        self.processed_file = os.path.join(self.processed_dir, f"{self.symbol}_{self.frequency}min_processed.parquet")
